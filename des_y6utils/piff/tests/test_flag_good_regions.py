...
def test_flag_good_regions_make_a_good_box_cen():
    bad_msk = np.zeros((13, 15)).astype(bool)
    bad_msk[6, 7] = True
    b, flag = _make_a_good_box(bad_msk)
    assert flag == 0
    assert b == {
...
def test_flag_good_regions_make_a_good_box_cen_inv():
    bad_msk = np.ones((13, 15)).astype(bool)
    bad_msk[6, 7] = False
    b, flag = _make_a_good_box(bad_msk)
    assert flag == 0
    assert b == {
